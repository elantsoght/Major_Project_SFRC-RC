...
from eng_module import SFRC, beams
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Moment-Curvature", "Beam Analysis", "Moment Check", "Shear Check"]
)
...
with tab1:
    ...
    st.write(
        f"The cracking moment is {Mphi[0][0]:.2f} kNm and the curvature at cracking is {Mphi[1][0]:.3e}."
    )
    st.write(
        f"The yield moment is {Mphi[0][1]:.2f} kNm the curvature at yielding is {Mphi[1][1]:.3e}."
    )
    st.write(
        f"The ultimate moment is {Mphi[0][2]:.2f} kNm the curvature at ultimate is {Mphi[1][2]:.3e}."
    )
    ...
    fig = go.Figure(
        data=[
            go.Scatter(
                x=[0, Mphi[1][0], Mphi[1][1], Mphi[1][2]],
                y=[0, Mphi[0][0], Mphi[0][1], Mphi[0][2]],
            )
        ]
    )
    ...
    fig.update_xaxes(
        zeroline=True, zerolinewidth=2, zerolinecolor="LightGray", range=[0, None]
    )
    fig.update_yaxes(
        zeroline=True, zerolinewidth=2, zerolinecolor="LightGray", range=[0, None]
    )
    fig.update_layout(
        plot_bgcolor="white",
        xaxis_showgrid=True,
        xaxis_gridcolor="rgb(245, 245, 245)",
        yaxis_showgrid=True,
        yaxis_gridcolor="rgb(245, 245, 245)",
    )
    ...
    st.write(
        "Mobasher, B., Y. Yao and C. Soranakom (2015). Analytical solutions for flexural design of hybrid steel fiber reinforced concrete beams. Engineering Structures 100: 164-177."
    )
...
with tab2:
    ...
    shearfactored = (
        1 / 1000 * dfactor * shearres["Dead"][0][1] + lfactor * shearres["Live"][0][1]
    )
    ...
    momentfactored = (
        ...
        / 1000
        * (
            1 / 1000 * dfactor * momentres["Dead"][0][1]
            + lfactor * momentres["Live"][0][1]
        )
    )
    ...
    with tab3:
        ...
        if designmoment <= Mphi[0][2]:
            st.write(
                f"The ultimate moment is {Mphi[0][2]:.2f} kNm and the factored moment demand is {designmoment:.2f} kNm."
            )
            st.write(
                "The demand is less than or equal to the capacity. The beam fulfills the requirements for flexure."
            )
            ...
            st.write(
                "The demand is larger than the capacity. The beam does not fulfill the requirements for flexure."
            )
        else:
    ...
    with tab4:
        ...
        st.write(
            f"The capacity provided by dowel action is {Vd:.2f} kN, or {percVd:.0f} percent of the total shear capacity."
        )
        st.write(
            f"The capacity provided by the concrete compression zone is {Vc:.2f} kN, or {percVc:.0f} percent of the total shear capacity."
        )
        st.write(
            f"The capacity provided by aggregate interlock is {Vai:.2f} kN, or {percVai:.0f} percent of the total shear capacity."
        )
        st.write(
            f"The capacity provided by the fibers bridging the crack is {VFiber:.2f} kN, or {percVFiber:.0f} percent of the total shear capacity."
        )
        ...
        fig = px.pie(
            values=values,
            names=labels,
            title="Contributions of shear-carrying mechanisms",
        )
        st.plotly_chart(fig)
        ...
        if VCSDT >= designshear:
            st.write(
                f"The shear capacity of {VCSDT:.2f} kN is larger than or equal to the factored shear demand {designshear:.2f} kN."
            )
            st.write(f"The SFRC-RC beam fulfills the requirements for shear.")
        elif VCSDT < designshear:
            st.write(
                f"The shear capacity of {VCSDT:.2f} kN is smaller than the factored shear demand {designshear:.2f} kN."
            )
        ...
        st.write(
            "Lantsoght, E. O. L. (2023). Theoretical model of shear capacity of steel fiber reinforced concrete beams. Engineering Structures. Vol. 280"
        )
