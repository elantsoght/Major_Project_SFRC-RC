...
def fe_model_ss_cant(
    ...
    beam_model.add_member("M0", "N0", "N2", "My material", Iy=1.0, Iz=I, J=J, A=A)
...
def build_beam(beam_data: dict) -> FEModel3D:
    ...
    """
    beam_data["Nodes"] = get_node_locations(
        list(beam_data["Supports"].keys()), beam_data["L"]
    )
    beam_model = FEModel3D()
...
def extract_arrays_all_combos(
    solved_beam_model=FEModel3D, result_type=str, direction=str, n_points=int
):
    """
    ...
    for combo_name, load_combo in solved_beam_model.LoadCombos.items():
        ...
        for member in solved_beam_model.Members.values():
            if result_type == "shear":
                ...
                result_array.append(
                    member.moment_array(direction, n_points, combo_name)
                )
                ...
                result_array.append(
                    member.deflection_array(direction, n_points, combo_name)
                )
